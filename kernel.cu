...
__device__ inline void sortMass(float arr[], int left, int right) {
	int i = left, j = right;
	int tmp;
...
void medianFilter(const Mat& input, Mat& output) {
	...
	cout << "Time on GPU: " << time << " ms" << endl;
}

int main() {

	Mat input = imread("1.bmp", IMREAD_GRAYSCALE);
